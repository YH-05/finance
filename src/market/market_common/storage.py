...
market.market_common.constants : TABLE_TICKERS and MarketExchange enum.
...
from market.market_common._utils import _coerce_optional_int, _coerce_optional_str
from market.market_common.constants import TABLE_TICKERS, MarketExchange
...
class AseanTickerStorage:
    ...
    def _build_ticker_df(tickers: list[TickerRecord]) -> pd.DataFrame:
        ...
        Resolves ``MarketExchange`` enum values to plain strings *before*
        ...
        for t in tickers:
            ...
            market = d["market"]
            d["market"] = (
                market.value if isinstance(market, MarketExchange) else str(market)
            )
            rows.append(d)
    ...
    def get_tickers(self, market: MarketExchange) -> list[TickerRecord]:
    ...
    def lookup_ticker(
        self,
        name: str,
        market: MarketExchange | None = None,
    ) -> list[TickerRecord]:
    ...
    def _df_to_ticker_records(df: pd.DataFrame) -> list[TickerRecord]:
        ...
        for row_dict in df.to_dict(orient="records"):
            market = MarketExchange(str(row_dict["market"]))
            records.append(
                TickerRecord(
