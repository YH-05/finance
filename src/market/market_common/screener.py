...
>>> from market.market_common.constants import MarketExchange
>>> tickers = fetch_tickers_from_screener(MarketExchange.SGX)
>>> len(tickers) > 0
True

See Also
--------
market.market_common.constants : SCREENER_MARKET_MAP and YFINANCE_SUFFIX_MAP.
market.market_common.types : TickerRecord dataclass.
market.market_common.errors : AseanScreenerError exception.
"""
...
def _query_screener(market: MarketExchange) -> tuple[int, pd.DataFrame]:
    """Execute a tradingview-screener query for a single market.

    Parameters
    ----------
    market : MarketExchange
        The exchange to query.
    ...
    Notes
    -----
    For ``MarketExchange.NSE``, an additional exchange filter
    (``exchange == "NSE"``) is applied to avoid duplicate results
    from both NSE and BSE-listed stocks sharing the same
    ``market="india"`` in tradingview-screener.
    """
    ...
    # AIDEV-NOTE: NSE deduplication — both NSE and BSE listings appear in
    # market="india". Apply exchange filter to return NSE-only results and
    # avoid duplicate tickers when multiple exchanges share the same market
    # country string (e.g. NSE and BSE both map to "india").
    if market == MarketExchange.NSE:
        nse_exchange = SCREENER_EXCHANGE_MAP[MarketExchange.NSE]
        query = (
            Query()
...
def fetch_tickers_from_screener(market: MarketExchange) -> list[TickerRecord]:
    ...
    ----------
    market : MarketExchange
        The ASEAN exchange to query (e.g. ``MarketExchange.SGX``).
...
def fetch_all_asean_tickers() -> dict[MarketExchange, list[TickerRecord]]:
    ...
    Returns
    -------
    dict[MarketExchange, list[TickerRecord]]
        Dictionary mapping each MarketExchange to its list of TickerRecords.
    ...
    >>> len(result) == 6
    True
    >>> all(isinstance(k, MarketExchange) for k in result)
    True
    """
    from concurrent.futures import ThreadPoolExecutor, as_completed

    logger.info("Fetching tickers for all ASEAN markets")
    result: dict[MarketExchange, list[TickerRecord]] = {}
