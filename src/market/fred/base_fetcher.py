...
class BaseDataFetcher(ABC):
    ...
    ...         return bool(symbol and symbol.isupper())
    """
    ...
    def normalize_dataframe(
        ...
        >>> normalized = fetcher.normalize_dataframe(raw_df, "GDP")
        ...
        if df.empty:
            logger.warning("Empty DataFrame received", symbol=symbol)
    ...
    def _validate_options(self, options: FetchOptions) -> None:
        ...
        if not options.symbols:
            ...
            raise FREDValidationError("At least one symbol must be provided")
