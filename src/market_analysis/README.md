...
| パッケージ | バージョン | 用途 |
|-----------|----------|------|
| `finance` | - | コアインフラ（DB接続、ロギング、ユーティリティ） |
...
| `fredapi` | >=0.5.0 | FRED 経済指標取得 |
| `plotly` | >=5.18.0 | チャート生成 |
| `matplotlib` | >=3.8.0 | グラフ可視化 |
| `scipy` | >=1.17.0 | 科学計算 |
| `statsmodels` | >=0.14.6 | 統計モデル |
| `scikit-learn` | >=1.8.0 | 機械学習 |
| `seaborn` | >=0.13.2 | 統計データ可視化 |

### オプションライブラリ

| パッケージ | バージョン | 用途 |
|-----------|----------|------|
| `blpapi` | >=3.24.0 | Bloomberg Terminal 連携（要ライセンス） |
| `pyarrow` | >=15.0.0 | Parquet フォーマット対応 |
| `kaleido` | >=1.2.0 | Plotly 静的画像エクスポート |
| `tenacity` | >=9.1.2 | リトライ処理 |
| `httpx` | >=0.28.1 | HTTP クライアント |
| `lxml` | >=6.0.2 | XML/HTML パース |
...
| `hypothesis` | >=6.150.2 | プロパティベーステスト |
| `ruff` | >=0.14.11 | リント・フォーマット |
| `pyright` | >=1.1.403 | 型チェック |
