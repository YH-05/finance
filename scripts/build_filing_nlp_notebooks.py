...
def build_01_fetch_and_chunk() -> nbf.NotebookNode:
    ...
    nb.cells = [
        ...
        _code(
            ...
            "# GLiNER の predict_entities は単一テキスト入力。\n"
            "# バッチ推論は batch_predict_entities (texts: list[str]) を使うと高速化。\n"
            "entity_rows = []\n"
            "texts = df_chunks['text'].tolist()\n"
            "meta = df_chunks[['filing_id','ticker','form','section_key','chunk_idx']].to_dict('records')\n"
            "\n"
            "BATCH_SIZE = 8  # GLiNER large は重いので小さめ\n"
            "for start in tqdm(range(0, len(texts), BATCH_SIZE), desc='gliner'):\n"
            "    batch_texts = texts[start:start+BATCH_SIZE]\n"
            "    # batch_predict_entities -> list[list[dict]]\n"
            "    batch_results = gliner_model.batch_predict_entities(\n"
            "        batch_texts, GLINER_LABELS, threshold=GLINER_THRESHOLD,\n"
            "    )\n"
            "    for i, ents in enumerate(batch_results):\n"
            "        m = meta[start + i]\n"
            "        for e in ents:\n"
