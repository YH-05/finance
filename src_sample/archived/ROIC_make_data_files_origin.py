...
def merge_weight_and_roic() -> pd.DataFrame:
    """
    インデックス構成銘柄をID別にsedolとcusipに分け, ROICデータとマージ
...
def add_past_and_forward_roic_cols_quarter(
    ...
    """
    add_past_and_forward_roic_cols
    過去 or 将来のROICの値を入れるカラムを追加する

    Args:
        df (pd.DataFrame): _description_
        quarter_period (int, optional): _description_. Defaults to 20.
...
def add_shifted_roic_cols_month(
    ...
    """
    add_shifted_roic_cols
    過去 or 将来のROICの値を入れるカラムを追加する

    Args:
        df (pd.DataFrame): _description_
        shift_direction (str, optional): _description_. Defaults to "both".
            options: "both", "forward", "past"
            1. "both" : 過去と将来のROICを追加
            2. "forward" : 将来のROICを追加
            3. "past" : 過去のROICを追加
        month_period (int, optional): _description_. Defaults to 60.
