...
class BlpapiCustom:
    ...
    # --------------------------------------------------------------------------
    def _create_session(self, verbose: bool = True) -> Optional[blpapi.Session]:
        """BLPAPIセッションを開始します。"""
    ...
    def get_financial_data(
        ...
        fiscal_period: Optional[str] = None,  # 1FY, 2FY, etc.
    ...
    def get_historical_news(
        ...
        """
        過去のBFW/BNニュースヘッドラインを取得

        :param source: "BFW" (Bloomberg First Word) または "BN" (Bloomberg News)
        :param days_back: 何日前まで遡るか
        :param securities: 特定証券でフィルタ (例: ["AAPL US Equity"])
        :param topics: トピックコードでフィルタ (例: ["HOT", "IMMWIN"])
        :param max_results: 最大取得件数
        :param verbose: ログ出力の有効/無効
        :return: ニュースヘッドラインのリスト
        """
        session = self._create_session(verbose=verbose)
        if not session:
            return []
        ...
        if topics:
            for topic in topics:
                request.append("topics", topic)

        if verbose:
            filter_info = ""
            if securities:
                filter_info += f" [証券: {len(securities)}件]"
            if topics:
                filter_info += f" [トピック: {', '.join(topics)}]"
            print(
                f"📡 {source}ニュースをリクエスト中 ({days_back}日前から){filter_info}..."
            )
                    ...
                    if msg.hasElement("newsHeadlines"):
                        ...
                        for headline in news_headlines.values():
                            headline_data = {
                                ...
                                "story_datetime": headline.getElementAsDatetime(
                                    "storyDateTime"
                                ),
                                "source": headline.getElementAsString("source"),
        ...
        if verbose:
            print(f"\n✅ {len(headlines)}件のニュースヘッドラインを取得しました。")

        return headlines

    # --------------------------------------------------------------------------
    def get_news_story_content(
        self, story_id: str, verbose: bool = True
    ) -> Optional[Dict[str, Any]]:
        """
        Story IDから記事本文を取得

        :param story_id: ニュース記事のID (例: "NS:20231115_NEWS_12345")
        :param verbose: ログ出力の有効/無効
        :return: 記事本文を含む辞書
        """
        session = self._create_session(verbose=verbose)
        ...
        news_service = session.getService(self.NEWS_SERVICE)
        request = news_service.createRequest("NewsStoryRequest")
        request.set("storyId", story_id)

        if verbose:
            print(f"📡 Story ID: {story_id} の本文を取得中...")
            ...
            if (
                ...
                for msg in event:
                    if msg.hasElement("story"):
                        story = msg.getElement("story")

                        story_content = {
                            "headline": story.getElementAsString("headline"),
                            "story_datetime": story.getElementAsDatetime(
                                "storyDateTime"
                            ),
                            "body": story.getElementAsString("body"),
                            "source": story.getElementAsString("source"),
                            "story_id": story.getElementAsString("storyId"),
                        }
    ...
    def get_multiple_sources_news(
        ...
        """
        複数ソースのニュースを一度に取得し、ソース別に分類

        :param sources: ソースのリスト (例: ["BFW", "BN"])
        :param days_back: 何日前まで遡るか
        :param max_results: 最大取得件数
        :param verbose: ログ出力の有効/無効
        :return: ソース別のニュース辞書
        ...
        if verbose:
            print(
                f"📡 複数ソース ({', '.join(sources)}) のニュースを取得中 ({days_back}日間)..."
            )
        ...
        categorized = {}
        for source in sources:
            categorized[source] = [h for h in headlines if h["source"] == source]

        if verbose:
            print("\n✅ ニュース取得完了:")
            for source in sources:
                print(f"   {source}: {len(categorized[source])}件")

        return categorized

    # --------------------------------------------------------------------------
    def get_historical_news_by_topic(
        self,
        topic_code: str,
        source: str = "BN",
        days_back: int = 7,
        ...
        :param topic_code: トピックコード (例: "HOT", "IMMWIN", "OIL", "TECH")
        :param source: "BFW" または "BN"
        :param days_back: 何日前まで遡るか
        :param max_results: 最大取得件数
        :param verbose: ログ出力の有効/無効
        :return: ニュースヘッドラインのリスト
        ...
        if verbose:
            print(
                f"📡 トピック '{topic_code}' のニュースを取得中 ({source}, {days_back}日間)..."
            )
        ...
        if verbose:
            print(f"✅ {len(headlines)}件のニュースを取得しました。")
    ...
    def compare_sector_news(
        ...
        :param sectors: セクターコードのリスト
        :param days_back: 何日前まで遡るか
        :param source: "BN" または "BFW"
        :param verbose: ログ出力の有効/無効
        ...
        for sector in sectors:
            if verbose:
                print(f"\n📊 セクター '{sector}' のニュースを取得中...")

            news = self.get_historical_news_by_topic(
                topic_code=sector, source=source, days_back=days_back, verbose=False
            )
            results[sector] = len(news)

            if verbose:
                print(f"   {sector}: {len(news)}件")

        df = pd.DataFrame(list(results.items()), columns=["Sector", "Article_Count"])
        df = df.sort_values("Article_Count", ascending=False)

        if verbose:
            print(f"\n✅ セクター別記事数比較完了 ({days_back}日間)")
    ...
    def analyze_news_trend(
        ...
        """
        キーワードを含むニュース記事数の推移を分析

        :param keyword: 検索キーワード
        :param days_back: 何日前まで遡るか
        :param source: "BN" または "BFW"
        :param save_chart: グラフをファイルに保存するか
        :param verbose: ログ出力の有効/無効
        :return: 日別記事数のSeries
        """
        # 過去のニュース取得
        bn_news = self.get_historical_news(
            source=source, days_back=days_back, verbose=verbose
        )
        ...
        # DataFrameに変換
        df = pd.DataFrame(bn_news)
        df["date"] = pd.to_datetime(df["story_datetime"]).dt.date

        # キーワードでフィルター
        df_filtered = df[df["headline"].str.contains(keyword, case=False, na=False)]

        # 日別集計
        daily_counts = df_filtered.groupby("date").size()

        if verbose:
            print(f"\n📈 キーワード '{keyword}' のトレンド分析:")
            print(f"   総記事数: {len(df_filtered)}件")
            print(f"   期間: {daily_counts.index.min()} - {daily_counts.index.max()}")
            if len(daily_counts) > 0:
                print(f"   1日平均: {daily_counts.mean():.1f}件")
                print(f"   最大: {daily_counts.max()}件 ({daily_counts.idxmax()})")

        # グラフ化
        if save_chart and len(daily_counts) > 0:
            try:
                import matplotlib.pyplot as plt

                plt.figure(figsize=(12, 6))
                daily_counts.plot(kind="line", marker="o")
                plt.title(f'News Trend: "{keyword}" ({source})')
                plt.xlabel("Date")
                plt.ylabel("Article Count")
                plt.grid(True)
                plt.tight_layout()

                filename = f"news_trend_{keyword.replace(' ', '_')}_{source}.png"
                ...
                if verbose:
                    print(
                        "⚠️ matplotlibがインストールされていないため、グラフは保存されませんでした。"
                    )
    ...
    def store_to_database(
        ...
        """
        DataFrameをSQLiteデータベースに保存します。

        Parameters
        ----------
        df : pd.DataFrame
            データベースに保存するデータを含むDataFrame。
        db_path : Path
            SQLiteデータベースファイルのパス。
        table_name : str
            データを保存するテーブル名。
        ...
        date_column : str, optional
            DataFrame内の日付カラム名。デフォルトは 'Date'。
        conflict_action : str, optional
            重複時の動作 ('IGNORE', 'REPLACE')。デフォルトは 'IGNORE'。
        verbose : bool, optional
            ログ出力の有効/無効
        """
        if df.empty:
            if verbose:
                print("⚠️ DataFrameが空のため、保存処理をスキップします。")
            return 0

        conn = sqlite3.connect(db_path)
        try:
            pk_definition = ", ".join([f'"{col}"' for col in primary_keys])

            columns_sql = []
            for col in df.columns:
                if col in primary_keys or col == date_column:
                    columns_sql.append(f'"{col}" TEXT NOT NULL')
                else:
                    dtype = (
                        "REAL" if df[col].dtype in (np.float64, np.float32) else "TEXT"
                    )
            ...
            print(f"❌ データベース構造エラーが発生しました: {e}")
            conn.close()
            return 0

        rows_affected = 0
        try:
            temp_table_name = f"{table_name}_temp_{np.random.randint(1000, 9999)}"
            ...
            action = "OR IGNORE"
            if conflict_action.upper() == "REPLACE":
                action = "OR REPLACE"
            ...
            if verbose:
                if rows_affected > 0:
                    print(
                        f"✅ 保存完了。テーブル '{table_name}' に {rows_affected} 行を処理しました ({conflict_action})。"
                    )
                else:
                    print(
                        f"ℹ️ 保存完了。テーブル '{table_name}' に変更はありませんでした。"
                    )
    ...
    def update_historical_data(
        ...
        id_type: str,
        field: str = "PX_LAST",
        default_start_date: Optional[datetime.datetime] = None,
        currency: Optional[str] = None,
        verbose: bool = True,
    ) -> int:
        """
