...
5. **チャンク設定**:
   ...
   - チャンク分割ルール: **ファイル単位でチャンクとする**（各ファイルが1つのルールを表すため、ファイルをまたいだ分割は不可）
   - 方法: 区切り文字を `---` に設定するか、各ファイルを個別にアップロード
...
3. チャンク設定:
   - セグメンテーション: **「自動」を推奨**（10-Kは長文のため）
   - 最大チャンク長: 1000〜2000トークン
   - Overlap: 100〜200トークン（セクション境界の文脈保持のため）

> **注意**: 10-Kは10万トークン超のため、自動分割が適切。Item 1, 1A, 7, 8などの主要セクションが適切に分割されているか確認すること。
