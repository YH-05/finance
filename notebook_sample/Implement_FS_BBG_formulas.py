...
def _(BPM_DATA_DIR, display, factset_utils):
    ...
    factset_utils.load_bpm_and_export_factset_code_file(
        index_dir=index_dir, start_date=_start_date, end_date=_end_date
    )
...
def _(
    ...
    for date in tqdm(df_members["date"].unique()):
        ...
        df_sedol = blp.load_ids_from_blpapi(
            id_type="SEDOL", id_list=_sedol_list, as_of_date=date
        )
        df_cusip = blp.load_ids_from_blpapi(
            id_type="CUSIP", id_list=_sedol_list, as_of_date=date
        )
        df_isin = blp.load_ids_from_blpapi(
            id_type="ISIN", id_list=_sedol_list, as_of_date=date
        )
...
def _(FACTSET_ROOT_DIR, display, pd):
    ...
    for _df in _dfs:
        ...
        g = pd.DataFrame(
            _df.groupby(["date", "Universe", "P_SYMBOL_MISS"])["Weight (%)"].agg(
                ["count", "sum"]
            )
        )
        display(g)
...
def _(FACTSET_ROOT_DIR, display, factset_utils):
    ...
    for _universe_code in universe_code_list:
        factset_utils.implement_factset_formulas(
            universe_code=_universe_code, year_range="20AY"
        )
...
def _(FACTSET_FINANCIALS_DIR, factset_utils):
    ...
    _file_list = list(
        _universe_folder.glob(f"D_Financials*_{_year_range}.xlsx")
    ) + list(_universe_folder.glob(f"D_Price_{_year_range}.xlsx"))
    factset_utils.format_factset_downloaded_data(
        file_list=_file_list, output_folder=_universe_folder, split_save_mode=True
    )  # type:ignore
...
def _(FACTSET_FINANCIALS_DIR, factset_utils):
    ...
    factset_utils.format_factset_downloaded_data(
        file_list=_file_list, output_folder=_universe_folder, split_save_mode=False
    )  # type: ignore
...
def _(BLOOMBERG_PRICE_DIR, FACTSET_ROOT_DIR, display, pd, utils):
    _universe_code = "MSXJPN_AD"
    _df_security = pd.read_parquet(
        FACTSET_ROOT_DIR
        / f"Index_Constituents/{_universe_code}_Index_Constituents_with_Factset_code.parquet",
        columns=["Asset ID", "SEDOL", "CUSIP", "ISIN", "CODE_JP", "Country"],
    )
    display(_df_security.shape)
    _sedol_list = list(
        set([s + " SEDOL" for s in _df_security["SEDOL"].tolist() if not pd.isna(s)])
    )
    _cusip_list = list(
        set([s + " CUSIP" for s in _df_security["CUSIP"].tolist() if not pd.isna(s)])
    )
    _isin_list = list(
        set([s + " ISIN" for s in _df_security["ISIN"].tolist() if not pd.isna(s)])
    )
    _code_jp_list = list(
        set(
            [s + " CODE_JP" for s in _df_security["CODE_JP"].tolist() if not pd.isna(s)]
        )
    )
...
def _(BLOOMBERG_PRICE_DIR, FACTSET_ROOT_DIR, implement_utils, pd):
    ...
    _code_jp_list = implement_utils.create_identifier_list(
        _df_security["CODE_JP"], "CODE_JP"
    )
    print("=== 識別子統計 ===")
    ...
    implement_utils.create_excel_with_chunked_data(
        _output_excel_file, identifier_dict, chunk_size=500
    )
...
def _(BLOOMBERG_PRICE_DIR, BPM_DATA_DIR, bpm_code_map, np, pd):
    ...
    _df["Weight (%)"] = _df["Weight (%)"].astype(float)
    all_columns = [
        col for col in _df.columns.tolist() if col not in ["SEDOL", "CUSIP", "ISIN"]
    ]
...
def _(MSCI_KOKUSAI_OUTPUT_DIR, display, pd, tqdm):
    ...
    for _symbol in tqdm(df_price_universe.columns.tolist()):
        ...
        _df_slice = pd.DataFrame(
            data={"date": _data_date, "value": _data_price}
        ).assign(P_SYMBOL=_symbol, variable="FG_PRICE")
        _dfs.append(_df_slice)
    df_kokusai = (
        pd.concat(_dfs, ignore_index=True)
        .dropna(subset=["date", "value"], how="all", ignore_index=True)
        .reindex(columns=["date", "P_SYMBOL", "variable", "value"])
    )
    display(df_kokusai.head())
    print(df_kokusai.shape)
    df_kokusai = df_kokusai.assign(
        date=lambda x: pd.to_datetime(x["date"])
    ...
    df_kokusai_1.to_parquet(
        MSCI_KOKUSAI_OUTPUT_DIR / "MSCI_KOKUSAI_Price_1.parquet", index=False
    )
    df_kokusai_2.to_parquet(
        MSCI_KOKUSAI_OUTPUT_DIR / "MSCI_KOKUSAI_Price_2.parquet", index=False
    )
    df_kokusai_3.to_parquet(
        MSCI_KOKUSAI_OUTPUT_DIR / "MSCI_KOKUSAI_Price_3.parquet", index=False
    )
    df_kokusai_4.to_parquet(
        MSCI_KOKUSAI_OUTPUT_DIR / "MSCI_KOKUSAI_Price_4.parquet", index=False
    )
    # export
    del (
        df_kokusai_1,
...
def _(MSCI_KOKUSAI_OUTPUT_DIR, display, pd, tqdm):
    ...
    df_benchmark.replace(
        {"991200": "MSCI Kokusai Index (World ex Japan)", "SP50": "S&P 500"},
        inplace=True,
    )
    ...
    df_benchmark.to_parquet(
        MSCI_KOKUSAI_OUTPUT_DIR / "Benchmark_Price.parquet", index=False
    )
