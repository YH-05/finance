...
def _(
    ...
    print(f'✅ インデックスデータ: {len(df_return_index):,}件')  # "Forward_Return_3M_annlzd",
...
def _(
    ...
    with sqlite3.connect(financials_db_path) as _conn:
        for _factor, _periods in tqdm(itertools.product(_factor_list, _period_list), total=_total_iterations, desc='ファクター処理進捗'):
            ...
            df_9 = pd.merge(df_weight_1, df_9, on=['date', 'P_SYMBOL'], how='outer').drop_duplicates(subset=['date', 'P_SYMBOL']).dropna(subset=['Weight (%)', _factor_growth], how='any', axis=0, ignore_index=True)
...
def _(
    ...
    with sqlite3.connect(bloomberg_valuation_db_path) as _conn:
        for _factor in tqdm(_factor_list):
            ...
            df_13 = pd.merge(df_weight_2, df_13, on=['date', 'SEDOL'], how='outer').drop_duplicates(subset=['date', 'SEDOL']).dropna(subset=['Weight (%)', _factor], how='any', axis=0, ignore_index=True)
...
def _(
    ...
    with sqlite3.connect(bloomberg_valuation_db_path) as _conn:
        for _factor, _periods in tqdm(itertools.product(_factor_list, _period_list), total=_total_iterations, desc='ファクター処理進捗'):
            ...
            df_14 = pd.merge(df_weight_3, df_14, on=['date', 'SEDOL'], how='outer').drop_duplicates(subset=['date', 'SEDOL']).dropna(subset=['Weight (%)', _factor_growth], how='any', axis=0, ignore_index=True)
            display(df_14)
    return
...
def _(
    ...
    _security_info = factset_utils.load_index_constituents(factset_index_db_path=factset_index_db_path, UNIVERSE_CODE=UNIVERSE_CODE)
    df_roic_merged = factset_utils.merge_idx_constituensts_and_financials(df_weight=_security_info, df_factor=df_roic_and_roe).assign(ROIC=lambda x: np.where(x['GICS Sector'] == 'Financials', x['FF_ROE'], x['FF_ROIC'])).dropna(subset=['Weight (%)', 'ROIC'], how='any', ignore_index=True).drop(columns=['FF_ROIC', 'FF_ROE'])
    _year_period = 3
    df_roic_merged = roic_utils.add_factor_rank_cols(df_roic_merged, factor_name='ROIC').rename(columns={'P_SYMBOL': 'Symbol'})
    df_roic_merged = roic_utils.add_shifted_factor_cols_month(df_roic_merged, factor_name='ROIC_Rank', shift_month=list(range(1, int(_year_period * 12) + 1)), shift_direction='Past').rename(columns={'Symbol': 'P_SYMBOL'})
    _roic_label_name = f'ROIC_label_Past{_year_period}Y'
    df_roic_merged[_roic_label_name] = df_roic_merged.apply(lambda row: roic_utils.test_assign_roic_label(row=row, freq='annual', shift_direction='Past', year_period=_year_period, judge_by_slope=False), axis=1)
    df_15 = df_roic_merged.copy()
    df_15 = df_15[['date', 'P_SYMBOL', _roic_label_name]].rename(columns={_roic_label_name: 'value'}).dropna(subset=['value'], ignore_index=True).assign(variable=_roic_label_name, date=lambda row: pd.to_datetime(row['date']))
    display(df_15.tail(10))
    factset_utils.store_to_database(df=df_15, db_path=financials_db_path, table_name=_roic_label_name)
    return
...
def _(
    ...
    with sqlite3.connect(factset_index_db_path) as _conn:
        ...
        _security_info = pd.read_sql(_query, con=_conn).rename(columns={'P_SYMBOL': 'Symbol'}).assign(date=lambda row: pd.to_datetime(row['date']))
    df_roic_merged_1 = pd.merge(df_roic, _security_info, on=['date', 'Symbol'], how='left').assign(ROIC=lambda x: np.where(x['GICS Sector'] == 'Financials', x['FF_ROE'], x['FF_ROIC'])).dropna(subset=['Weight (%)', 'ROIC'], how='any').drop(columns=['FF_ROIC', 'FF_ROE'])
    df_roic_merged_1 = roic_utils.add_factor_rank_cols(df_roic_merged_1, factor_name='ROIC')
    df_roic_merged_1 = roic_utils.add_shifted_factor_cols_month(df_roic_merged_1, factor_name='ROIC_Rank', shift_month=list(range(1, 61)), shift_direction='Past')
    _year_period = 5
    _roic_label_name = f'ROIC_label_Past{_year_period}Y'
    df_roic_merged_1[_roic_label_name] = df_roic_merged_1.apply(lambda row: roic_utils.test_assign_roic_label(row=row, freq='annual', shift_direction='Past', year_period=_year_period, judge_by_slope=False), axis=1)
    df_16 = df_roic_merged_1.copy()
    df_16 = df_16[['date', 'Symbol', _roic_label_name]].rename(columns={_roic_label_name: 'value', 'Symbol': 'P_SYMBOL'}).dropna(subset=['value'], ignore_index=True).assign(variable=_roic_label_name, date=lambda row: pd.to_datetime(row['date']))
    display(df_16)
    factset_utils.store_to_database(df=df_16, db_path=financials_db_path, table_name=_roic_label_name)
...
def _(df_roic_merged_1, display, pd):
    ...
    roic_count = pd.pivot(pd.DataFrame(df_17.groupby(['date', 'GICS Sector', 'ROIC_label_Past5Y'])['Symbol'].count()).reset_index(), index=['date', 'GICS Sector'], columns='ROIC_label_Past5Y').reset_index()
    # --- label count ---
    display(roic_count.loc[roic_count['GICS Sector'] == 'Information Technology'])
    roic_count = pd.pivot(pd.DataFrame(df_17.groupby(['date', 'GICS Sector', 'ROIC_Rank'])['Symbol'].count()).reset_index(), index=['date', 'GICS Sector'], columns='ROIC_Rank').reset_index()
    display(roic_count.loc[roic_count['GICS Sector'] == 'Information Technology'])
    weight_total_count = df_17.groupby(['date'])['Weight (%)'].agg(['count', 'sum']).rename(columns={'count': 'Num of Securities', 'sum': 'Total Weight (%)'}).sort_index()
    weight_sector_count = df_17.groupby(['date', 'GICS Sector'])['Weight (%)'].agg(['count', 'sum']).rename(columns={'count': 'Num of Securities', 'sum': 'Total Weight (%)'}).sort_index()
    display(weight_total_count)
    display(weight_sector_count)
    roic_label_count = df_17.groupby(['date', 'ROIC_label_Past5Y'])['Weight (%)'].agg(['count', 'sum']).rename(columns={'count': 'Num of Securities', 'sum': 'Total Weight (%)'}).sort_index()
    # --- weight check ---
...
def _(
    ...
    df_weight_4 = factset_utils.load_index_constituents(factset_index_db_path=factset_index_db_path, UNIVERSE_CODE=UNIVERSE_CODE)
    _df_factor = factset_utils.load_financial_data(financials_db_path=financials_db_path, factor_list=['FF_ROIC_Rank', 'FF_ROE_Rank'])
    df_roic_rank = factset_utils.merge_idx_constituensts_and_financials(df_weight=df_weight_4, df_factor=_df_factor).assign(ROIC_Rank=lambda x: np.where(x['GICS Sector'] == 'Financials', x['FF_ROE_Rank'], x['FF_ROIC_Rank'])).drop(columns=['FF_ROIC_Rank', 'FF_ROE_Rank']).rename(columns={'SEDOL': 'Symbol'})
...
def _(df_roic_rank, display, pd):
    ...
    display(_g)
    g_count = pd.DataFrame(df_roic_rank.groupby(['date', 'GICS Sector'])['SEDOL'].count()).reset_index().pivot(index=['date'], columns='GICS Sector', values='SEDOL')
    display(g_count)
    return
...
def _(
    ...
    with sqlite3.connect(financials_db_path) as _conn:
        for _factor in tqdm(_factor_list):
            df_18 = pd.read_sql(f'SELECT `date`, `P_SYMBOL`, `value` FROM `{_factor}`', con=_conn, parse_dates=['date']).assign(date=lambda row: pd.to_datetime(row['date']) + pd.tseries.offsets.MonthEnd(0)).sort_values('date', ignore_index=True).rename(columns={'value': _factor})
            df_18 = pd.merge(df_weight_5, df_18, on=['date', 'P_SYMBOL'], how='outer').drop_duplicates(subset=['date', 'P_SYMBOL']).dropna(subset=['Weight (%)', _factor], how='any', axis=0, ignore_index=True)
            ...
            _df_pct_rank = roic_utils.add_factor_pct_rank_cols(df=df_18, factor_name=_factor)
            ...
            _df_zscore = roic_utils.add_factor_zscore_cols(df=df_18, factor_name=_factor)
...
def _(
    ...
    with sqlite3.connect(factset_index_db_path) as _conn:
        df_19 = pd.read_sql(f'SELECT * FROM {UNIVERSE_CODE} LIMIT 5', parse_dates=['date'], con=_conn)
        display(df_19)
...
def _(
    ...
    with sqlite3.connect(factset_index_db_path) as _conn:
        df_weight_6 = pd.read_sql(f'SELECT * FROM {UNIVERSE_CODE}', parse_dates=['date'], con=_conn)
...
def _(
    ...
    with ThreadPoolExecutor(max_workers=4) as executor:
        future_to_var = {executor.submit(process_variable, var, financials_db_path, df_weight_6): var for var in table_names_5}
...
def _(
    ...
    with sqlite3.connect(financials_db_path) as _conn:
        for _variable in table_names_5:
            _df_factor = pd.read_sql(f'SELECT `date`, `P_SYMBOL`, `value` FROM `{_variable}`', con=_conn, parse_dates=['date'])
            merged_df = pd.merge(df_weight_6, _df_factor, on=['date', 'P_SYMBOL'], how='outer').rename(columns={'value': _variable}).dropna(subset=['Weight (%)', _variable], how='any', axis=0).fillna(np.nan)
            _g = pd.DataFrame(merged_df.groupby(['date'])['Weight (%)'].agg('sum')).reset_index().assign(variable=_variable)
...
def _(
    ...
    df_20 = pd.merge(df_weight_7, _data, on=['date', 'P_SYMBOL'], how='outer').dropna(subset=['Weight (%)']).fillna(np.nan)
    display(df_20)
...
def _(df_20, display):
    ...
    display(_g.tail(10))
    display(df_20.loc[df_20['FG_COMPANY_NAME'].str.contains('NVIDIA') & (df_20['GICS Sector'] == 'Information Technology')])
    return
...
def _(df_20, display, pd, roic_utils):
    avg_return = pd.DataFrame(df_20.groupby(['date', 'ROIC_label_Past5Y']).apply(lambda row: roic_utils.clipped_mean(row['Return_Ann_5Y'], percentile=5))).reset_index().rename(columns={0: 'avg'})
    std = pd.DataFrame(df_20.groupby(['date', 'ROIC_label_Past5Y']).apply(lambda row: roic_utils.clipped_std(row['Return_Ann_5Y'], percentile=5)).reset_index().rename(columns={0: 'std'}))
