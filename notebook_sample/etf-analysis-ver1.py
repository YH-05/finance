...
def _(ETF_DIR, curl_cffi, display, np, pd, yf):
    ...
    df_1 = pd.melt(
        data_1,
        id_vars=["date", "Ticker"],
        value_vars=data_1.columns.tolist()[2:],
        var_name="variable",
        value_name="value",
    ...
    log_return_wide = df_1[df_1["date"] >= _start_date].pivot(
    ...
    df_corr = pd.melt(
        _corr_matrix,
        id_vars="Ticker1",
        value_vars=_corr_matrix.columns.tolist()[1:],
        var_name="Ticker2",
...
def _(datetime, df_1, display, pd, tqdm):
    ...
    for end_date in tqdm(date_range):
        ...
        log_return_wide_1 = df_1.loc[
            (df_1["date"] >= _start_date) & (df_1["date"] < end_date)
        ].pivot(index="date", columns="Ticker", values="log_return")
        _corr_matrix = (
            log_return_wide_1.corr().reset_index().rename(columns={"Ticker": "Ticker1"})
        )
        df_corr_1 = pd.melt(
            _corr_matrix,
            id_vars="Ticker1",
        ...
        df_corr_1 = df_corr_1.sort_values("correlation", ignore_index=True).assign(
            date=end_date
        )
...
def _(go, log_return_wide_1, np, pd):
    ...
    benchmark_beat_performers = cum_return_latest.loc[
        cum_return_latest[cum_return_latest["Ticker"] == "^SPX"].index[0] + 1 :,
        "Ticker",
    ].tolist()
    _fig = go.Figure()
