...
def _(analyzer, df):
    ...
    _df_to_plot = analyzer.calculate_mean_returns_for_roic_label(df=df, factor_col=roic_factor_name, return_col=return_col)
    analyzer.plot_roic_label_performance(df_to_plot=_df_to_plot, roic_factor_name=roic_factor_name, return_col=return_col)
    return
...
def _(analyzer, df):
    ...
    _df_to_plot = analyzer.calculate_mean_returns_for_roic_label(df=df, factor_col=roic_factor_name_1, return_col=return_col_1)
    analyzer.plot_roic_label_performance(df_to_plot=_df_to_plot, roic_factor_name=roic_factor_name_1, return_col=return_col_1)
    return
...
def _(alpha_sectors, analyzer, df, non_alpha_sectors):
    ...
    _df_performance = analyzer.calculate_mean_returns_for_roic_label(df_alpha_sectors, factor_col=roic_factor_name_2, return_col=return_col_2)
    analyzer.plot_roic_label_performance(df_to_plot=_df_performance, roic_factor_name=roic_factor_name_2, return_col=return_col_2)
    print('Non-Alpha Sectors')
    df_non_alpha_sectors = df.loc[df['GICS Sector'].isin(non_alpha_sectors)].reset_index(drop=True)
    _df_performance = analyzer.calculate_mean_returns_for_roic_label(df_non_alpha_sectors, factor_col=roic_factor_name_2, return_col=return_col_2)
    analyzer.plot_roic_label_performance(df_to_plot=_df_performance, roic_factor_name=roic_factor_name_2, return_col=return_col_2)
...
def _(df_alpha_sectors, display, pd, return_col_2, roic_utils):
    def display_performance_table(df_target: pd.DataFrame):
        count_by_sector = pd.DataFrame(df_target.groupby(['GICS Sector'])['P_SYMBOL'].count().sort_values())
        display(count_by_sector)
        return_mean_col = f'{return_col_2}_mean'
        return_std_col = f'{return_col_2}_std'
        efficiency_col = 'Efficiency'
        eff_by_sector = df_target.groupby(['GICS Sector']).agg({return_col_2: [(return_mean_col, roic_utils.clipped_mean), (return_std_col, roic_utils.clipped_std)]}).droplevel(0, axis=1).assign(**{efficiency_col: lambda x: x[return_mean_col].div(x[return_std_col])}).sort_values(by=efficiency_col, ascending=True)
        display(eff_by_sector)
    alpha_sector_and_remain_high = df_alpha_sectors.loc[(df_alpha_sectors['ROIC_label_Past3Y'] == 'remain high') & (df_alpha_sectors['year'] == 2025)]
    alpha_sector_and_drop_to_low = df_alpha_sectors.loc[(df_alpha_sectors['ROIC_label_Past3Y'] == 'drop to low') & (df_alpha_sectors['year'] == 2025)]
    pd.options.display.precision = 3
    print('===== remain high =====')
    display_performance_table(alpha_sector_and_remain_high)  # 1. GICS Sectorでグループ化
    print('===== drop to low =====')  # type: ignore
    display_performance_table(alpha_sector_and_drop_to_low)  # 2. aggを使用して、平均(mean)と標準偏差(std)を同時に計算  # type:ignore  # 3. リターンを標準偏差で割ってEfficiencyを計算し、カラムを追加  # 4. Efficiencyでソート
    return
...
def _(go, pd):
    ...
    def plot_dispersion(median_factor: pd.DataFrame, factor_name: str, sector_name: str):
        ...
        for col in ['Dispersion_Q1_Q2', 'Dispersion_Q1_Q3', 'Dispersion_Q1_Q4']:
            fig.add_trace(go.Scatter(x=median_factor.index, y=median_factor[col], mode='lines', name=col, line=dict(width=0.5)))
        fig.update_layout(title=f'{_factor_name} Dispersion ({sector_name})', yaxis_title='dispersion', hovermode='x', width=1000, height=300, template='plotly_dark', legend=dict(yanchor='top', y=-0.1, xanchor='center', x=0.5, orientation='h'), margin=dict(l=30, r=30, t=50, b=40))
        fig.show()
...
def _(alpha_sectors, analyzer_1, df_1):
    ...
    _df_performance = analyzer_1.calculate_mean_returns_double_factors(df=df_alpha_sectors_1, factor1=roic_factor_name_3, factor2=_factor_name, return_col=return_col_3, factor1_rank='remain high')
    analyzer_1.plot_factor_performance(df_to_plot=_df_performance, factor_name=_factor_name, return_col=return_col_3)
    return
...
def _(alpha_sectors, analyzer_1, df_1):
    ...
    _df_performance = analyzer_1.calculate_mean_returns_double_factors(df=df_alpha_sectors_2, factor1=roic_factor_name_4, factor2=_factor_name, return_col=return_col_4, factor1_rank='drop to low')
    analyzer_1.plot_factor_performance(df_to_plot=_df_performance, factor_name=_factor_name, return_col=return_col_4)
    return
...
def _(db_utils, display, financials_db_path, pd, sqlite3):
    ...
    factor_list = [s for s in tables if s.startswith('FF_') and s.endswith('PctRank')] + [roic_factor_name_5]
    TARGET_YEAR = '2025'
    ...
    # クエリ全体を結合
    df_factors = df_2.loc[df_2['variable'] != roic_factor_name_5].reset_index(drop=True).pivot(index=['date', 'P_SYMBOL'], columns='variable', values='value')
    df_roic = df_2.loc[df_2['variable'] == roic_factor_name_5].reset_index(drop=True).pivot(index=['date', 'P_SYMBOL'], columns='variable', values='value')
    df_final = pd.merge(df_factors, df_roic, left_index=True, right_index=True).reset_index()
    display(df_final)
