...
def _():
    ...
    from fred_database_utils import (  # type: ignore
...
def _(List, Path, SECURITY_DATA_DIR, TICKER, display, pd):
    def read_excel_data(
        excel_file: Path, sheet_name: str, variable: str
    ) -> pd.DataFrame:
        df = pd.melt(
    ...
    def read_excel_data_serial(
        ...
        dfs = []
        for sheet_name, variable in zip(sheet_name_list, variable_list, strict=False):
            df = read_excel_data(
    ...
    df = df.loc[df["Ticker"] != "GOOGL US Equity"].sort_values(
        "date", ignore_index=True
    )
    display(
        df.loc[
            (df["variable"] == "Price") & (df["Ticker"] == "ABNB US Equity")
        ].dropna()
    )
    return (df,)
    ...
    for _ticker in df_cum_return.columns:
        ...
        _fig.add_trace(
            go.Scatter(
                ...
                y=df_cum_return[_ticker],
                name=_name,
                line=dict(width=_line_width, color=_line_color),
            )
        )
    _fig.update_layout(
        width=1000,
...
def _(TICKER, df, go, make_subplots, pd, start_date):
    ...
    for _ticker in df_actual_pe.columns:
        ...
        _fig.add_trace(
            go.Scatter(
                ...
                y=df_actual_pe[_ticker],
                name=_ticker,
                line=dict(width=0.8, color=_line_color),
            )
        )
    _fig.update_yaxes(range=(0, 60))
    _fig.update_layout(
    ...
    for _ticker in [
        ...
        df_forward_and_actual_pe = df[
            ...
            & df["Ticker"].str.startswith(_ticker)
        ]
        ...
        _fig.update_layout(
            ...
            hovermode="x unified",
            legend=dict(
                yanchor="top", y=-0.05, xanchor="center", x=0.5, orientation="h"
            ),
            margin=dict(l=40, r=40, t=50, b=60),
            title=f"{_ticker} : Actual PE LTM and Forward PE NTM",
        )
        _fig.show()
