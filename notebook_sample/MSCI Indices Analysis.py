...
def _(db_path, np, pd, plt, sns, sqlite3):
    _conn = sqlite3.connect(db_path)
    _df_price = pd.read_sql("SELECT * FROM FG_PRICE", con=_conn).drop(
        columns=["variable"]
    )
    _df_price = pd.pivot(
        _df_price, index="date", columns="Index", values="value"
    ).sort_index()
    ...
    _fig.suptitle(
        f"MSCI Index Cumulative Return (start={start_date.strftime('%Y-%m-%d')})"
    )
...
def _(INDEX_DIR, display, np, pd, plt, sns, sqlite3):
    ...
    _df = pd.merge(df_eps, _df_price, left_index=True, right_index=True).fillna(
        method="ffill"
    )
    _df = _df.assign(
