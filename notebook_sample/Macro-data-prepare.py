...
def _(BBG_DIR, display, pd, sqlite3):
    ...
    df_1 = (
        pd.merge(df_1, df_ticker, on=["Index"], how="left")
        .drop(columns=["Index"])
...
def _(BBG_DIR, FRED_DIR, display, pd, sqlite3):
    ...
    _conn = sqlite3.connect(_db_path)
    df_yield = pd.read_sql("SELECT * FROM T10Y2Y", con=_conn, index_col="date").assign(
    ...
    df_2 = pd.pivot(df_2, index="date", columns="Index", values="value").assign(
        ...
        .mean(),
        Service_EMA=lambda row: row["ISM Services PMI"]
        .ewm(span=6, adjust=False)
        .mean(),
    )
    display(df_2)
