...
def _():
    ...
    from us_treasury import plot_us_interest_rates_and_spread, analyze_yield_curve_pca, plot_loadings_and_explained_variance
...
def _(DATA_DIR, createDB_bpm_and_factset_code, display, pd):
    ...
    df_1 = pd.concat(_dfs).assign(date=lambda row: pd.to_datetime(row['date'])).sort_values('date', ignore_index=True)
    g1 = df_1.groupby(['date'])['Weight (%)'].agg(['count', 'sum']).rename(columns={'count': 'num of secs', 'sum': 'total weight (%)'})
    display(g1)
    display(g1.describe())
    g2 = df_1.groupby(['date', 'GICS Sector'])['Weight (%)'].agg(['count', 'sum']).rename(columns={'count': 'num of secs', 'sum': 'total weight (%)'})
    display(g2)
    _db_name = DATA_DIR / 'MSCI_KOKUSAI_BPM_and_Factset_Code.db'
    createDB_bpm_and_factset_code(db_name=_db_name, raw_data_list=_parquet_files, file_type='parquet')
    return
...
def _(DATA_DIR, Path, pd, sqlite3, tqdm):
    ...
    df_copy = pd.pivot_table(df_copy, index=['date', 'P_SYMBOL'], columns='variable', values='value').reset_index().sort_values(['date', 'P_SYMBOL'], ignore_index=True).assign(date=lambda row: pd.to_datetime(row['date'].dt.strftime('%Y-%m-%d')))
    shift_cols = [col for col in df_copy.columns if not col in ['P_SYMBOL', 'date']]
    concat_df = df_copy.copy()
    for shift_period in tqdm(range(1, 61)):
        shifted_values_past = df_copy.groupby('P_SYMBOL')[shift_cols].shift(periods=shift_period).add_suffix(f'_{shift_period}M_ago')
        shifted_values_future = df_copy.groupby('P_SYMBOL')[shift_cols].shift(periods=-shift_period).add_suffix(f'_{shift_period}M_forward')
    ...
    try:
        ...
        for _date in unique_dates:
            ...
            print(f"日付: {_date.date()} のデータをテーブル '{table_name}' に書き込みました。")
...
def _(DATA_DIR, pd, sqlite3, tqdm):
    def _step1_load_csv_to_db(csv_files, db_path):
        ...
        try:
            ...
            print(f"全CSVデータをデータベース '{_db_path.name}' の 'raw_data' テーブルに保存しました。")
        except Exception as e:
    ...
    def step2_process_and_create_features(db_path):
        ...
        try:
            _conn = sqlite3.connect(_db_path)
            symbols = pd.read_sql_query('SELECT DISTINCT P_SYMBOL FROM raw_data', _conn)['P_SYMBOL'].tolist()
            for _i, symbol in enumerate(tqdm(symbols, desc='Processing Symbols')):
                ...
                df_pivot = pd.pivot_table(df_symbol, index=['date', 'P_SYMBOL'], columns='variable', values='value').reset_index()
                shift_cols = [col for col in df_pivot.columns if col not in ['P_SYMBOL', 'date']]
                all_features_long = []
                base_features = df_pivot.melt(id_vars=['date', 'P_SYMBOL'], value_vars=shift_cols, var_name='feature_name', value_name='value')
                ...
                for shift_period in range(1, 61):
                    ...
                    past_shift_long = pd.concat([df_pivot[['date', 'P_SYMBOL']], past_shift], axis=1).melt(id_vars=['date', 'P_SYMBOL'], value_vars=shift_cols, var_name='feature_name', value_name='value')
                    past_shift_long['feature_name'] = past_shift_long['feature_name'] + f'_{shift_period}M_ago'
                    all_features_long.append(past_shift_long)
                    future_shift = df_pivot[shift_cols].shift(periods=-shift_period)
                    future_shift_long = pd.concat([df_pivot[['date', 'P_SYMBOL']], future_shift], axis=1).melt(id_vars=['date', 'P_SYMBOL'], value_vars=shift_cols, var_name='feature_name', value_name='value')
                    future_shift_long['feature_name'] = future_shift_long['feature_name'] + f'_{shift_period}M_forward'
                ...
                final_symbol_df_long.to_sql('final_data_long_format', _conn, if_exists=write_mode, index=False)
            print(f"全銘柄の処理が完了し、'final_data_long_format' テーブルに保存されました。")
        except Exception as e:
...
def _(DATA_DIR, pl, step2_process_and_create_features):
    ...
    def _step1_load_csv_to_db(csv_glob_path, db_uri):
        ...
        try:
            ...
            df_raw = df_raw.with_columns(pl.col('date').str.to_datetime().dt.strftime('%Y-%m-%d'))
            df_raw.write_database('raw_data', db_uri, if_table_exists='replace')  # 日付のフォーマットを統一
            print(f"全CSVデータを 'raw_data' テーブルに保存しました。")
        except Exception as e:
            print(f'ステップ1でエラーが発生しました: {e}')
    _csv_files = sorted(list(DATA_DIR.glob('Financials_all_data*.csv')))
    csv_glob_path = str(DATA_DIR) + '/Financials_all_data*.csv'  # 'raw_data'テーブルとして保存
    _db_name = DATA_DIR / 'MSCI_KOKUSAI_Factset_Financials.db'
...
def _(
    ...
    step1_load_file_to_db(file_list=_parquet_files, file_type='parquet', db_name=_db_path)
...
def _(DATA_DIR, display, get_table_names, pd, sqlite3, tqdm):
    ...
    _variables = [s for s in get_table_names(db_uri=db_uri) if (s != 'all_data') & ~s.endswith('_processed')]
    ...
    for _variable in tqdm(_variables[:3]):
        ...
        df_3 = pd.read_sql(sql=_query, con=_conn).sort_values('date').rename(columns={'value': _variable})
        for _i in range(-60, 61):
            if _i == 0:
                continue
            col_name = f'{_variable}_{abs(_i)}Mago' if _i > 0 else f'{_variable}_{abs(_i)}Mforward'
            df_3[col_name] = df_3.groupby('P_SYMBOL')[_variable].shift(_i)
        df_3 = pd.melt(df_3, id_vars=['date', 'P_SYMBOL'], value_name='value', var_name='variable')
        display(df_3)
        df_3.to_sql(name=f'{_variable}_processed', con=_conn, if_exists='replace', index=False, method='multi')
    _conn.close()
    return
...
def _(DATA_DIR, calculate_missing_stats, get_table_names, pd, sqlite3, tqdm):
    ...
    exclude_columns = ['Bloomberg Ticker', 'BloombergID', 'CODE_JP', 'P_SYMBOL_SEDOL', 'P_SYMBOL_CUSIP', 'P_SYMBOL_ISIN', 'FG_COMPANY_NAME_SEDOL', 'FG_COMPANY_NAME_CUSIP', 'FG_COMPANY_NAME_ISIN', '欠損状態']
    ...
    for _variable in tqdm([s for s in tables_financials if s != 'all_data']):
        _query = f"\n    SELECT\n        date,\n        P_SYMBOL,\n        value\n    FROM\n        '{_variable}'\n    "
        _df_financials = pd.read_sql(sql=_query, con=_conn).rename(columns={'value': _variable})
        _df_financials = _df_financials.sort_values('date', ignore_index=True).assign(year=lambda row: pd.to_datetime(row['date']).dt.year).assign(month=lambda row: pd.to_datetime(row['date']).dt.month)  # 除外するカラム
        _df_financials = pd.merge(_df_financials, _df_code.drop(columns=['date']), on=['year', 'month', 'P_SYMBOL'], how='outer').dropna(subset=['date', 'Weight (%)'], how='any', ignore_index=True)
        missing_stats = calculate_missing_stats(group_cols=['date'], df=_df_financials, variable=_variable)
        missing_stats_dfs.append(missing_stats)
        missing_stats_sector = calculate_missing_stats(group_cols=['date', 'GICS Sector'], df=_df_financials, variable=_variable)
    ...
    missing_stats.to_csv(DATA_DIR / 'Factset_Financials_Missing_data_report.csv', index=False)
    # export missing value report
    missing_stats_sector.to_csv(DATA_DIR / 'Factset_Financials_Missing_data_sector_report.csv', index=False)  # インデックス構成銘柄に対して欠損している銘柄数とウェイトを計算したデータフレームを格納  # インデックス構成銘柄に対して欠損している銘柄数とウェイトをセクター別に計算したデータフレームを格納
    return
...
def _(BM_DIR, display, pd, sqlite3):
    ...
    for sheet_name in sheet_names:
        df_4 = pd.read_excel(BM_DIR / 'Benchmark_Price_and_Valuation.xlsx', sheet_name=sheet_name).dropna(subset=['date'], ignore_index=True)
        df_melt = pd.melt(df_4, id_vars='date', var_name='Benchmark', value_name='value').assign(variable=sheet_name).reindex(columns=['date', 'Benchmark', 'variable', 'value'])
        _dfs.append(df_melt)
...
def _(BM_DIR, mdates, pd, plt, sns, sqlite3):
    ...
    _db_path = BM_DIR / 'Benchmark_Price_and_Valuation.db'
    _conn = sqlite3.connect(_db_path)
    df_5 = pd.pivot(pd.read_sql(sql=f"SELECT * FROM '{_benchmark}'", con=_conn), index='date', columns='variable', values='value').sort_index()
    ...
    for _i, _variable in enumerate([s for s in _variables if not s in ['FMA_EVAL_EBIT', 'FMA_EVAL_EBITDA', 'FMA_EVAL_SALES']]):
        _ax = _axes[_i]
...
def _(DATA_DIR, display, get_table_names, pd, sqlite3):
    ...
    # display(financials_db_date_list[0])
        ...
        _df_financials = pd.pivot_table(_df_financials, index=['date', 'P_SYMBOL'], columns='variable', values='value').reset_index().sort_values('date', ignore_index=True)
        display(_df_financials)
        merged_df = pd.merge(_df_financials, _df_code[['date', 'P_SYMBOL', 'GICS Sector', 'GICS Industry', 'Weight (%)']], on=['date', 'P_SYMBOL'], how='left').dropna(subset='Weight (%)')
...
def _(FRED_DIR, store_fred_database):
    ...
    yield_data = ['DFF', 'DGS1MO', 'DGS3MO', 'DGS6MO', 'DGS1', 'DGS2', 'DGS3', 'DGS5', 'DGS7', 'DGS10', 'DGS20', 'DGS30']
    spread = ['T10Y2Y', 'T10Y3M']  # FF金利（実行金利）
    store_fred_database(db_path=_db_path, series_id_list=yield_data + spread)  # 1 month  # 3 month  # 6 month  # 1 year  # 2 year  # 3 year  # 5 year  # 7 year  # 10 year  # 20 year  # 30 year
    return
...
def _(FRED_DIR, pd, plot_loadings_and_explained_variance, sqlite3):
    series_id_list = ['DGS1MO', 'DGS3MO', 'DGS6MO', 'DGS1', 'DGS2', 'DGS3', 'DGS5', 'DGS7', 'DGS10', 'DGS20', 'DGS30']
    _conn = sqlite3.connect(FRED_DIR / 'FRED.db')
    _dfs = []
    for series_id in series_id_list:
        df_8 = pd.read_sql(sql=f"SELECT * FROM '{series_id}'", con=_conn).rename(columns={series_id: 'value'}).assign(variable=series_id)
        _dfs.append(df_8)
    df_8 = pd.pivot(pd.concat(_dfs, ignore_index=True), index='date', values='value', columns='variable').sort_index()
...
def _(DATA_DIR, display, np, pd, sqlite3):
    ...
    _conn = sqlite3.connect(_db_path)
    _variable = 'FF_ROIC'
    df_10 = pd.read_sql(f"SELECT * FROM '{_variable}'", con=_conn, parse_dates=['date']).assign(year=lambda row: row['date'].dt.year, month=lambda row: row['date'].dt.month).drop(columns='variable')
    _db_path = DATA_DIR / 'MSCI_KOKUSAI_BPM_and_Factset_Code.db'
    _conn = sqlite3.connect(_db_path)
    _df_code = pd.read_sql(sql='SELECT * FROM all_data', con=_conn)
    df_10 = pd.merge(df_10, _df_code[['year', 'month', 'P_SYMBOL', 'Weight (%)', 'GICS Sector']], how='left').dropna(subset=['Weight (%)'])
    df_10[f'{_variable}_rank_universe'] = df_10.groupby('date')['value'].transform(lambda x: pd.qcut(x, q=5, labels=False, duplicates='drop') + 1 if x.notna().sum() >= 5 else pd.Series(np.nan, index=x.index)).replace({1.0: 'rank5', 2.0: 'rank4', 3.0: 'rank3', 4.0: 'rank2', 5.0: 'rank1'})
    df_10[f'{_variable}_rank_sector'] = df_10.groupby(['date', 'GICS Sector'])['value'].transform(lambda x: pd.qcut(x, q=5, labels=False, duplicates='drop') + 1 if x.notna().sum() >= 5 else pd.Series(np.nan, index=x.index)).replace({1.0: 'rank5', 2.0: 'rank4', 3.0: 'rank3', 4.0: 'rank2', 5.0: 'rank1'})
    df_10.rename(columns={'value': _variable}, inplace=True)
    display(df_10)
    _db_path = DATA_DIR / 'MSCI_KOKUSAI_Factset_Financials.db'
    _conn = sqlite3.connect(_db_path)
    df_return = pd.pivot(pd.read_sql(sql='SELECT * FROM Return', con=_conn, parse_dates=['date']), index=['date', 'P_SYMBOL'], columns='variable', values='value').reset_index()
...
def _(df_10, mdates, pd, plt):
    _variable = 'FF_ROIC'
    g = pd.DataFrame(df_10.groupby(['date', f'{_variable}_rank_universe'])['P_SYMBOL'].agg('count')).reset_index()
    g = pd.pivot(g, index='date', columns=f'{_variable}_rank_universe', values='P_SYMBOL')
    g['total_secs'] = g.sum(axis=1)
    g_weight = pd.DataFrame(df_10.groupby(['date', f'{_variable}_rank_universe'])['Weight (%)'].agg('sum')).reset_index()
    g_weight = pd.pivot(g_weight, index='date', columns=f'{_variable}_rank_universe', values='Weight (%)')
    ...
    _ax = g_weight[cols_to_plot].plot(kind='area', stacked=True, figsize=(10, 4), alpha=0.7)
