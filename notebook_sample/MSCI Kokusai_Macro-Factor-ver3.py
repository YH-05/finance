...
def _(FRED_DIR, display, pd, return_secs, sqlite3):
    _db_path = FRED_DIR / 'FRED.db'
    _conn = sqlite3.connect(_db_path)
    risk_free_rate = pd.read_sql('SELECT * FROM DTB3', con=_conn, parse_dates=['date']).assign(DTB3=lambda row: row['DTB3'].div(365 * 100))
    _conn.close()
...
def _(
    ...
    df_factor = pd.merge(excess_return_index.set_index('date')[['Factor_Market']], df_yield, left_index=True, right_index=True)
    ...
    plot_loadings_and_explained_variance(df_yield=df_factor[[s for s in df_factor.columns if s.startswith('DGS')]])
    _df_pca, _pca = analyze_yield_curve_pca(df_yield=df_factor[[s for s in df_factor.columns if s.startswith('DGS')]])
    _df_pca = _df_pca.assign(Level_Shock=lambda row: row['Level'].diff(), Slope_Shock=lambda row: row['Slope'].diff(), Curvature_Shock=lambda row: row['Curvature'].diff()).dropna()
    df_factor = pd.merge(df_factor, _df_pca, left_index=True, right_index=True)
    df_factor = df_factor[[col for col in df_factor.columns if ('Factor' in col) | ('Shock' in col)]]
...
def _(df_factor, display, orthogonalize):
    # 金利ファクター直行化
    df_factor['Factor_Level'] = orthogonalize(df_factor['Level_Shock'], df_factor['Factor_Market'])
    df_factor['Factor_Slope'] = orthogonalize(df_factor['Slope_Shock'], df_factor['Factor_Market'])
    df_factor['Factor_Curvature'] = orthogonalize(df_factor['Curvature_Shock'], df_factor['Factor_Market'])
    regressors_ftoq = df_factor[['Factor_Market', 'Factor_Level', 'Factor_Slope', 'Factor_Curvature']]
    df_factor['Factor_FtoQ'] = orthogonalize(df_factor['FtoQ_Shock'], regressors=regressors_ftoq)
    regressors_inflation = df_factor[['Factor_Market', 'Factor_Level', 'Factor_Slope', 'Factor_Curvature', 'Factor_FtoQ']]
    df_factor['Factor_Inflation'] = orthogonalize(df_factor['Inflation_Shock'], regressors=regressors_inflation)
    df_factor_1 = df_factor[['Factor_Market', 'Factor_Level', 'Factor_Slope', 'Factor_Curvature', 'Factor_FtoQ', 'Factor_Inflation']].dropna(how='any')
    display(df_factor_1)
...
def _(df_factor_1, display, excess_return_secs, pd):
    security = 'BKNG-US'
    excess_return_security = excess_return_secs.loc[excess_return_secs['P_SYMBOL'] == security, ['date', 'Excess_Return']].set_index('date')
    display(excess_return_security)
    df_data = pd.merge(excess_return_security, df_factor_1, left_index=True, right_index=True)
    display(df_data)
    return
...
def _(PCA, orthogonalize, pd, sm, tqdm):
    ...
    def calculate_rolling_betas(stock_price_series, df_raw_macro, window_size, yield_cols=['DGS1MO', 'DGS3MO', 'DGS6MO', 'DGS1', 'DGS2', 'DGS3', 'DGS5', 'DGS7', 'DGS10', 'DGS20', 'DGS30']):
        """
        ...
        results = []
        for t in tqdm(range(window_size, len(data)), desc=f'Rolling Betas for {stock_price_series.name}'):
            ...
            df_pc = pd.DataFrame(pc_scores_aligned, index=df[yield_cols].index, columns=['Level', 'Slope', 'Curvature'])
            level_shock = df_pc['Level'].diff()
            slope_shock = df_pc['Slope'].diff()
            curvature_shock = df_pc['Curvature'].diff()
            factor_level = orthogonalize(level_shock, factor_market)  # 株価リターンと超過リターンを計算
            ...
            factor_ftoq = orthogonalize(ftoq_shock, pd.concat([factor_market, factor_level, factor_slope, factor_curvature], axis=1))  # 分析に必要なデータを全て結合
            inflation_shock = df['T10YIE'].diff()
            all_other_factors = pd.concat([factor_market, factor_level, factor_slope, factor_curvature, factor_ftoq], axis=1)
            factor_inflation = orthogonalize(inflation_shock, all_other_factors)  # 結果を格納するリスト
            X = pd.concat([factor_market, factor_level, factor_slope, factor_curvature, factor_ftoq, factor_inflation], axis=1)
            X.columns = ['Market', 'Level', 'Slope', 'Curvature', 'FtoQ', 'Inflation']
            analysis_data = pd.concat([df['Excess_Return'], X], axis=1).dropna()  # ローリング処理
        ...
        beta_history = pd.DataFrame(results).set_index('date')  # 1. マーケットファクター
...
def _(
    ...
    df_descriptor = make_descriptor_dataframe(fred_db_path=fred_db_path, benchmark_db_path=benchmark_db_path)
    ...
    plot_loadings_and_explained_variance(df_yield=df_descriptor[[s for s in df_descriptor.columns if s.startswith('DGS')]])
    _df_pca, _pca = analyze_yield_curve_pca(df_yield=df_descriptor[[s for s in df_descriptor.columns if s.startswith('DGS')]])
    _df_pca = _df_pca.assign(Level_Shock=lambda row: row['Level'].diff(), Slope_Shock=lambda row: row['Slope'].diff(), Curvature_Shock=lambda row: row['Curvature'].diff()).dropna()
    df_descriptor = pd.merge(df_descriptor, _df_pca, left_index=True, right_index=True)
    df_descriptor = df_descriptor[[col for col in df_descriptor.columns if ('Factor' in col) | ('Shock' in col)]]
...
def _(df_factor_2, display, plt, sns):
    ...
    for i, col in enumerate(df_factor_2.columns.tolist()):
        ax = axes[i]
        sns.lineplot(df_factor_2, x=df_factor_2.index, y=df_factor_2[col], alpha=0.7, ax=ax)
        ax.set_ylabel(col)
    plt.show()
    return
