...
def _(
    ...
    with ProcessPoolExecutor(max_workers=_MAX_WORKERS) as _executor:
        _futures = {
            _executor.submit(get_etf_fundamentals, _ticker): _ticker
            for _ticker in _ticker_list
        }
        for _future in tqdm(
            as_completed(_futures), total=len(_ticker_list), desc="スクレイピング進捗"
        ):
            try:
                df = _future.result()
                if not df.empty:
...
def _(
    ...
    with ProcessPoolExecutor(max_workers=_MAX_WORKERS) as _executor:
        _futures = {
            _executor.submit(get_etf_fundamentals, _ticker): _ticker
            for _ticker in _missing_ticker_list
        }
        for _future in tqdm(
            as_completed(_futures),
            total=len(_missing_ticker_list),
            desc="スクレイピング進捗",
        ):
            try:
                df_2 = _future.result()
...
def _(
    ...
    try:
        ...
        _classification_data = _scrape_key_value_data(
            _driver, _wait, "classification-index-data"
        )
...
def _(
    ...
    try:
        ...
        try:
            ...
            _accept_button = _wait.until(
                EC.element_to_be_clickable((By.XPATH, _cookie_button_xpath))
...
def _(
    ...
    def _scrape_key_value_data(driver, wait, container_id: str) -> dict:
        ...
        try:
            ...
            print(
                f"  エラー: ID='{container_id}'のデータ取得中にエラーが発生しました: {e}"
            )
        return data_dict
    ...
    def scrape_all_tickers(tickers: list) -> pd.DataFrame:
        ...
        try:
            ...
            try:
                _accept_button = cookie_wait.until(
            ...
            for _ticker in tickers:
                ...
                try:
                    ...
                    print("  コンテンツが表示されました。")
                    _summary_data = _scrape_key_value_data(
                        _driver, _wait, "summary-data"
                    )
                    _classification_data = _scrape_key_value_data(
                        _driver, _wait, "classification-index-data"
                    )
                    if not _summary_data and (not _classification_data):
                        print(
                            f"'{_ticker}' のデータが取得できなかったため、スキップします。"
                        )
                        continue
                    _combined_data = {**_summary_data, **_classification_data}
                    ...
                    print(
                        f"--- ticker '{_ticker}' の処理中にエラーが発生しました: {e} ---"
                    )
                    continue
        finally:
    ...
    _ticker_list = (
        pd.read_sql("SELECT * FROM Ticker", con=_conn)["Ticker"].unique().tolist()
    )
    _conn.close()
...
def _(
    ...
    def get_fund_flows_data(ticker: str) -> pd.DataFrame:
        ...
        try:
            ...
            display_100_button = _wait.until(
                EC.presence_of_element_located(
                    (By.XPATH, "//button/span[text()='100']")
                )
            )
            _driver.execute_script(
                ...
                for row in rows:
                    ...
                    if len(cols) == 2:
                        ...
                        all_rows_data.append(
                            {"Date": date, f"{_ticker} Net Flows": net_flows}
                        )
                try:
                    next_button = _driver.find_element(
                        By.XPATH,
                        "//li[not(contains(@class, 'disabled'))]/a[text()='Next']",
                    )
                    _driver.execute_script(
        ...
        df[f"{_ticker} Net Flows"] = (
            df[f"{_ticker} Net Flows"].str.replace(",", "").astype(float)
        )
        df = (
            df.rename(columns={"Date": "date", f"{_ticker} Net Flows": "ETF Net Flows"})
            .assign(Ticker=_ticker)
