...
def _(df_yield, df_yield_copy, gridspec, pd, plt, sns):
    df_yield_ann = (
        df_yield_copy.groupby(df_yield_copy.index.year).tail(1).dropna(how="any")
    )
    ...
    gs_yield_curve = gridspec.GridSpecFromSubplotSpec(
        1, 1, subplot_spec=gs_master[0, 0]
    )
...
def _(analyze_yield_curve_pca, df_return, df_yield, display, pd, sm):
    ...
    _df_return_ex_benchmark = df_return.drop(
    ...
    _df_merged = pd.merge(
        _kokusai_return,
        _df_return_ex_benchmark[[_symbol]],
        left_index=True,
        right_index=True,
    )
    _df_merged = pd.merge(
        _df_merged, _df_pca, left_index=True, right_index=True
...
def _(analyze_yield_curve_pca, df_return, df_yield, pd, sm, tqdm):
    ...
    for _symbol in tqdm(_df_return_ex_benchmark.columns.tolist()):
        ...
        if len(_df_merged) != 0:
            _X = _df_merged[
                ["MSCI Kokusai Index (World ex Japan)", "PC1", "PC2", "PC3"]
            ]
            _Y = _df_merged[_symbol]
            _X = sm.add_constant(_X)
            _model = sm.OLS(_Y, _X).fit()
