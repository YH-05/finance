...
1. `/new-package <package_name>` でパッケージを作成
2. `/new-project @src/<library_name>/docs/project.md` 実行
3. インタビュー（最低 5 回の質問）で要件を詳細化
4. **project.md を作成/更新**
...
4. **プロジェクトファイルの存在を確認**:
   - **存在しない場合**: ステップ 1-0 でインタビューを行い、新規作成する
   - **存在する場合**: 内容を読み込み、ステップ 1-1 に進む

### ステップ 1-0: プロジェクトファイルの作成（存在しない場合のみ）

**project.md が存在しない場合**、インタビューを通じて新規作成する。

1. AskUserQuestion ツールを使用して、パッケージの概要をヒアリング:
   - パッケージの目的は？
   - 解決したい課題は？
   - 主要な機能は？
   - 技術的な制約や考慮事項は？

2. **project-file スキル**をロードし、テンプレートに従って `src/<library_name>/docs/project.md` を作成

3. 作成した内容をユーザーに提示し、確認を得る
