...
├─ 引数なし → エラー表示
├─ @ で始まる引数:
│   ├─ @src/*/docs/project.md パターンに一致 → パッケージ開発モード（従来）
│   └─ パターン不一致 → エラー表示（@ 引数エラー）
...
期待される形式: @src/<library_name>/docs/project.md

例:
  /new-project @src/market_analysis/docs/project.md

パッケージが未作成の場合は、先に /new-package を実行してください。
軽量プロジェクトを作成する場合は @ なしで実行してください:
  /new-project "プロジェクト名"
```
...
|------|--------|
| 最大文字数 | 2000文字 |
| 禁止文字 | 制御文字（`\x00`-`\x1F`、`\x7F`。ただし改行`\n`とタブ`\t`は許可） |
| サニタイズ | 制御文字を空文字に置換後、前後の空白をトリム |

**検証失敗時のエラーメッセージ:**
```
入力が長すぎます。2000文字以内で入力してください。
現在の文字数: {入力文字数}
```
...
プロジェクト名は**許可リスト方式**で検証する（ブロックリスト方式より安全）:

| 項目 | ルール |
|------|--------|
| 正規表現（許可パターン） | `^[\p{L}\p{N}\s\-_]{1,100}$` |
| 許可文字 | 英数字（`a-zA-Z0-9`）、日本語（ひらがな・カタカナ・漢字）、ハイフン（`-`）、アンダースコア（`_`）、スペース |
| 最小長 | 1文字 |
| 最大長 | 100文字 |

**検証ロジック（許可リスト方式）:**
```
1. 入力文字列の長さが1-100文字かチェック
2. 各文字が許可パターンに一致するかチェック:
   - Unicode Letter (\p{L}): 英字、日本語文字
   - Unicode Number (\p{N}): 数字
   - スペース、ハイフン、アンダースコア
3. 一致しない文字が1つでもあれば検証失敗
```

**検証失敗時のエラーメッセージ**:
```
エラー: プロジェクト名に使用できない文字が含まれています。
...
プロジェクト名から URL セーフなスラッグを生成する。

**スラッグ生成アルゴリズム:**

1. **日本語の英語変換**:
   - LLM（Claude）を使用してプロジェクト名を英語に翻訳
   - 翻訳結果を単語に分解
   - 3-5単語程度に要約
...
3. **セキュリティ対策（パストラバーサル防止）**:
   - `.` `/` `..` を完全に除去
   - URLエンコード形式（`%2F`, `%2E` 等）をデコード後に除去
   - Unicodeホモグリフ（見た目が似た文字）を正規化

4. **最終検証**:
   - パターン: `^[a-z0-9][a-z0-9-]*[a-z0-9]$`
   - 長さ: 3文字以上、30文字以内
   - 検証失敗時はユーザーに英語スラッグの入力を求める
...
```
生成されたスラッグが検証に失敗しました。
英語のスラッグを入力してください（3-30文字、英小文字・数字・ハイフンのみ）:
```
...
各タスクを GitHub Issue として作成。**シェルエスケープのため必ず HEREDOC 形式を使用**:
...
**重要**: HEREDOC の `'EOF'` はシングルクォートで囲むこと（変数展開を防止）。
