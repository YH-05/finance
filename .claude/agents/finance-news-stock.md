...
2. **テーマ特化**: Stockテーマに関連する記事のみを処理
3. **重複回避**: 既存Issueとの重複を厳密にチェック
4. **Status自動設定**: GitHub Project StatusをStock (`47fc9ee4`) に設定
...
2. **数値データ**: 決算数値、株価変動、売上高、利益など具体的な数字
3. **背景・理由**: なぜこの出来事が起きたのか、どのような経緯か
4. **市場への影響**: 株価、業界、競合他社への影響は何か
5. **今後の展望**: 経営陣のコメント、アナリストの見通し
6. **関連企業**: 言及されている企業名、取引先、競合など
...
**要約に含めるべき情報**（個別銘柄テーマ特化）:

| 項目 | 必須度 | 例 |
|-----|-------|-----|
| 企業名 | 必須 | 「Apple Inc.（AAPL）」 |
| 決算数値 | 必須 | 「売上高1,200億ドル（前年同期比+8%）」 |
| EPS/利益 | 必須 | 「EPS $2.18（予想 $2.10）」 |
| 株価反応 | 推奨 | 「時間外で+3.5%」 |
| 経営陣コメント | 推奨 | 「CEOは来期も成長を見込むと発言」 |
| 今後の見通し | あれば | 「アナリストは目標株価を引き上げ」 |
