...
**担当RSSフィードから直接記事を取得**し、個別銘柄関連のニュースを
...
**設定ソース**: `data/config/finance-news-themes.json` → `themes.stock.feeds`

セッションファイル（`.tmp/news-collection-{timestamp}.json`）の `feed_assignments.stock` から動的に読み込まれます。
...
3. **テーマ特化**: Stockテーマに関連する記事のみを処理
4. **重複回避**: 既存Issueとの重複を厳密にチェック
5. **Status自動設定**: GitHub Project StatusをStock (`47fc9ee4`) に設定
...
ASSIGNED_FEEDS = session_data["feed_assignments"]["stock"]
# 形式: [{"feed_id": "...", "title": "CNBC - Earnings"}, ...]
...
入力:
{json.dumps({"articles": articles_to_fetch, "theme": "stock"}, ensure_ascii=False, indent=2)}
...
| フィールド | 値 |
|-----------|-----|
| 要約 | {japanese_summary}（**400字以上の詳細要約**） |
| 情報源URL | {link} |
| 公開日 | {published_jst}(JST) |
| 収集日時 | ${collected_at}(JST)【必須】 |
| カテゴリ | Stock（個別銘柄） |
| フィード名 | {source} |
| 備考・メモ | テーマ・AI判定理由 |
...
[INFO] 担当フィードをフェッチ中...
[INFO] フェッチ成功: CNBC - Earnings (4件)
[INFO] フェッチ成功: CNBC - Business (6件)
[INFO] 記事取得完了: 10件
[INFO] 既存Issue取得中...
...
[INFO] 重複チェック中...
[INFO] 重複スキップ: トヨタ、四半期決算 → Issue #192 (URL一致)
[INFO] 重複スキップ: ソニー決算発表 → Issue #189 (タイトル類似(87%))
[INFO] 新規記事: 3件
...
| Issue # | タイトル | 公開日 |
|---------|----------|--------|
| #205 | トヨタ、決算を発表 | 2026-01-22 |
| #206 | ソフトバンク、ARM買収を完了 | 2026-01-22 |
| #207 | 三菱商事、上方修正を発表 | 2026-01-22 |
