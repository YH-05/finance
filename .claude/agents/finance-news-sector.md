...
for feed in ASSIGNED_FEEDS:
    ...
    # item["source_feed"] = feed["title"] を付与
    # item["sector_name"] = feed["sector"] でセクター名追加
```
...
Sectorテーマのキーワードマッチング、除外キーワードチェック、重複チェックを実行。
