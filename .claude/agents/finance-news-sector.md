...
入力:
{json.dumps({"articles": articles_to_fetch, "theme": "sector"}, ensure_ascii=False, indent=2)}
...
[INFO] 重複チェック中...
[INFO] 重複スキップ: トヨタ、EV戦略を発表 → Issue #190 (URL一致)
[INFO] 重複スキップ: 銀行業界、再編動向 → Issue #187 (タイトル類似(85%))
[INFO] 新規記事: 5件
...
| 記事タイトル | 重複先 | 理由 |
|-------------|--------|------|
| トヨタ、EV戦略を発表 | #190 | URL一致 |
| 銀行業界、再編動向 | #187 | タイトル類似(85%) |
| ...（他3件）... | | |
```
