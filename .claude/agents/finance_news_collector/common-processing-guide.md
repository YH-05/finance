...
> **🚨 重要 🚨**: WebFetchは記事内容の取得にのみ使用します。
> WebFetchがリダイレクト先URLを返しても、**Issue記載のURLはRSSオリジナルのlinkを使用**してください。
> WebFetchの戻り値URLは無視し、RSSから取得したURLをそのまま保持すること。
...
def fetch_article_content(url: str, title: str) -> str:
    ...
    - **重要**: WebFetchがリダイレクト情報を返しても、Issue記載の
      URLはこの関数に渡されたオリジナルURLを使用すること。
    """
