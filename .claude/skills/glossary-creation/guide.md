...
**定義**: タスク管理のために使用する GitHub の Issue 機能

**説明**:
GitHub Issues で実装タスクを管理します。`/issue` コマンドで project.md と
双方向同期し、`feature-implementer` でTDDベースの実装を行います。
Issue のチェックボックスで進捗を追跡します。

**関連用語**:

-   [project.md](#projectmd): プロジェクトの実装計画
-   [feature-implementer](#feature-implementer): TDD実装エージェント

**使用例**:

-   「機能追加のために Issue を作成する」
-   「Issue のチェックボックスで進捗を管理する」

**コマンド**:

- `/issue @src/<package_name>/docs/project.md` - Issue 管理・同期
...
**本プロジェクトでの用途**:
全てのソースコードを Python で記述し、型ヒントで型安全性を確保しています。
...
-   読みやすく保守しやすいコード
-   豊富なエコシステムとライブラリ
-   PEP 695 による最新の型ヒント機能

**代替技術**:

-   Node.js: 非同期処理に強いが Python の方が読みやすい
-   Go: パフォーマンスは高いが開発速度では Python に劣る
