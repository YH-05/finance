...
For each issue in issue_numbers:
    │
    ├─ issue-implementer サブエージェントを起動
    │   └─ Task ツールで委譲（独立したコンテキスト）
    │
    │   サブエージェント内:
    │   ├─ Issue #{n} の実装（Phase 0-5）
    │   └─ コミット作成（PR作成なし）
...
**重要**: 各IssueをTaskツールで `issue-implementer` サブエージェントに委譲することで、
コンテキストが自動的に分離されます。これにより `/clear` コマンドを使用する必要がなくなります。
...
### 0.6 サブエージェント委譲によるコンテキスト分離

各Issue実装時に `issue-implementer` サブエージェントを起動することで、コンテキストが自動的に分離されます。

**サブエージェント起動方法**:

```yaml
# Task ツールで issue-implementer を起動
