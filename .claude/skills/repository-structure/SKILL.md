...
**重要**: `src/<library_name>/docs/repository-structure.md` に既存のリポジトリ構造定義書がある場合、以下の優先順位に従ってください:

1. **既存のリポジトリ構造定義書 (`src/<library_name>/docs/repository-structure.md`)** - 最優先

    - ライブラリ固有のディレクトリ構造が記載されている
    - このスキルのガイドより優先する
