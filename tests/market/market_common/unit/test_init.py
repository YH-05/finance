"""Tests for market.market_common.__init__.py public API exports.

Validates that:
- All market_common public API symbols are importable from ``market.market_common``.
- MarketExchange, TickerRecord, AseanTickerStorage and other key types are accessible.
- Error classes and screener functions are re-exported.
- __all__ is complete and consistent.

Test TODO List:
- [x] market_common public API importability
- [x] __all__ completeness
- [x] Key types (MarketExchange, TickerRecord) importable
...
class TestAseanCommonPackageExports:
    """Test that market.market_common re-exports all public API symbols."""

    def test_正常系_MarketExchangeがインポート可能(self) -> None:
    ...
    def test_正常系_DB_PATHがインポート可能(self) -> None:
        ...
        from market.market_common import DB_PATH

        assert isinstance(DB_PATH, Path)

    def test_正常系___all__が定義されている(self) -> None:
        import market.market_common as asean_mod
    ...
    def test_正常系___all__の全シンボルが実際にインポート可能(self) -> None:
        ...
        for name in asean_mod.__all__:
            assert hasattr(asean_mod, name), (
                f"{name} is in __all__ but not importable from market.market_common"
            )
    ...
    def test_正常系_再エクスポートされた型が元モジュールと同一(self) -> None:
        ...
        from market.market_common import MarketExchange as ReExported
        from market.market_common.constants import MarketExchange as Original

        assert ReExported is Original

    def test_正常系_再エクスポートされたTickerRecordが元モジュールと同一(self) -> None:
        from market.market_common import TickerRecord as ReExported
        from market.market_common.types import TickerRecord as Original
    ...
    def test_正常系_再エクスポートされたAseanTickerStorageが元モジュールと同一(
        ...
        from market.market_common import AseanTickerStorage as ReExported
        from market.market_common.storage import AseanTickerStorage as Original
