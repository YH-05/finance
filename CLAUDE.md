...
| 並行開発一括作成   | `/create-worktrees <issue_number1> <issue_number2>` コマンド（複数worktree一括作成） |
| 並行開発一括削除   | `/delete-worktrees <branch_name1> <branch_name2>` コマンド（複数worktree一括削除） |
...
| 全工程一括実行     | `/finance-full` コマンド（フォルダ作成→リサーチ→執筆）     |
| マーケットレポート | `/generate-market-report` コマンド（データ収集→ニュース検索→レポート生成） |
...
## スキル・エージェント・スラッシュコマンドについて
### AIエージェントが使用するためのpythonスクリプト
- 背景: AIエージェントがpythonスクリプトを直接使用する場合は、特定の処理目的が事前にあるものとし、それは`.claude/skills/`に事前定義されたもののみ実行する。
- **禁止**:pythonスクリプトを`src/`以下のパッケージ・モジュールと、`.claude/`以下のディレクトリ以外の場所に作成すること
- **スキルにコードを組み込むことは許可する**: AIエージェントが直接使用するためのpythonスクリプトはすべてスキルに内包されたものとする。
