...
pythonスクリプトは`src/`以下のパッケージ・モジュールと、`.claude/`以下のディレクトリ以外の場所に作成することを**禁止する**。AIエージェントがpythonスクリプトを直接使用する場合は、特定の処理目的が事前にあるものとし、それは`.claude/skills/`に事前定義されたもののみ実行する。すなわち、AIエージェントが直接使用するためのpythonスクリプトはすべてスキルに内包されたものとする。
