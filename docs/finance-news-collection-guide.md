...
{
  "filtering": {
    "min_reliability_score": 3  // デフォルトは2。より厳格に
  }
}
```
...
1. **信頼性スコア**: 数値フィールド（0-100）
2. **カテゴリ**: 単一選択（株式、為替、金融政策、企業、etc.）
3. **地域**: 単一選択（日本、米国、欧州、アジア）
4. **優先度**: 単一選択（高、中、低）
