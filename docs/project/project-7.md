...
-   背景の種類: 新しいニーズが発生
-   詳細: 新機能の追加と既存機能の拡張
-   具体的な状況: note 記事用に画像を収集するエージェントを追加したい。特定サイトからデータを収集する機能が必要。

### 目的

新しいエージェントを `.claude/agents/` に追加し、note 記事作成のための画像収集・リサーチ機能を提供する。
...
| 種類         | 名前                     | 説明                              |
| ------------ | ------------------------ | --------------------------------- |
| エージェント | research-image-collector | note 記事用の画像収集エージェント |
| ドキュメント | エージェント使用ガイド   | 使い方と設定方法                  |

## 成功基準

-   [x] 特定の入力に対して期待する出力が得られる（画像 URL リストなど）
...
既存パターンに従う - リポジトリ内の類似エージェント（finance-web, finance-wiki 等）を参考に実装
...
-   [x] 既存リサーチ系エージェントの調査（finance-web, finance-wiki 等）
...
**最終更新**: 2026-01-16
**更新内容**: 追加タスク 2 件を Issue 化（#254, #255）、GitHub Project #7 に登録、ステータスを Todo に設定
